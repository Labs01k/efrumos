...
return [
    ...
    */
    ...
    'onec' => [
        ...
        // 2026-09-11: 1С прокомментировал каждый код из письма — Owner
        // зависит от способа доставки, не одно значение на все случаи (см.
        // efrumos-docs/open-decisions.md п.7 / efrumos-docs/1c-ws-amo-qa-2026-09-11.md):
        //   27416 (Client B2C)              — заказы с адресной доставкой
        //   27703 (Client B2C, fil. V.Alecsandri) — самовывоз (клиент сам забирает)
        //   28045 (Client B2C STRAUS.md)     — доставка конкретной курьерской
        //                                      службой (1С сами не уверены, актуально ли)
        //   27119 (Eliteh Trade SRL)         — "сам себе контрагент", похоже не для веб-заказов
        //
        ...
        // 28045 сопоставлен с delivery_method IN ('nova_courier','nova_terminal')
        // (Nova Poshta) чисто на будущее — эта опция сейчас скрыта в чекауте
        // (option закомментирован в checkout-form-items.blade.php), реальных
        // заказов с таким delivery_method не бывает, веткой ещё никто не
        // проходил ни разу.
        // Ни одно значение 1С явно не подтвердил как факт (сказали «для
        // теста ок, на проде см. выше» про 27416) — это наша лучшая
        // интерпретация их комментариев. 27119 не используем — нет
        // уверенного критерия, когда именно.
