...
return [
    ...
    'front' => [
        ...
        # Параметр «Применение» — вкладка на странице товара. Заведён патчем
        # database/sql/2026-09-08-usage-parametr.sql как textarea (свободный текст
        # на каждом языке). Вкладка показывается только у товаров с заполненным
        # текстом, так что наполнение можно вести постепенно.
        'usage_parametr_id' => 50,
        ...
        # Параметры «Цвет» и «Оттенок» в фильтре каталога. Когда покупатель
        # фильтрует по ним, линейки в выдаче не схлопываются: он ищет конкретный
        # оттенок, и одна карточка на линейку такой запрос не обслуживает.
        'shade_parametr_ids' => [10, 11],

        # Параметр «назначение» — участвует в подборе похожих товаров (п.4 ТЗ).
        # Это id=5 «Решение» / Beneficii (для восстановления, уход за окрашенными,
        # питание и увлажнение) — заполнен почти у всего каталога. Раньше тут
        # стоял id=1 «Для кого» / Pentru: он выключен в CMS и заполнен у 2 товаров
        # из 2919, из-за чего критерий назначения в подборе не работал вовсе.
        'purpose_parametr_id' => 5,
