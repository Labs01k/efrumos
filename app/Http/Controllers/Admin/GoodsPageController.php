...
class GoodsPageController extends Controller
{
    public function index()
    {
        ...
        foreach ($menu_id_elements as $key => $menu_id_element) {
            $menu_elements[$key] = GoodsPage::where('goods_page_id', $menu_id_element->id)
                ->first();
        }
        ...
        if ($imagePositions && !empty($imagePositions) && count($imagePositions)) {
            foreach ($imagePositions as $oneImagePosition) {
                if ($oneImagePosition['id'])
                    GoodsPageImages::find($oneImagePosition['id'])->update(['position' => $oneImagePosition['position']]);
            }
        }
    ...
    public function changeActive(Request $request)
    {
        ...
        if ($action == 'main-active')
            $element_id = GoodsPageId::findOrFail($id);
        else
            $element_id = '';
        ...
        if ($action == 'main-active')
            GoodsPageId::where('id', $id)->update(['active' => $change_active]);
    ...
    {
        $view = 'admin.goods-page.create-menu';

        $modules_name = $this->menu()['modules_name'];
        $curr_page_menu_id = GoodsPageId::where('alias', request()->segment(4))
            ->first();
        ...
        if (!is_null($curr_page_menu_id)) {
            ...
            $goods_subject_list = GoodsSubjectId::where('active', 1)
                ->where('deleted', 0)
                ->where('p_id', getMainCatalogId())
                ->has('itemByLang')
                ->with('itemByLang')
                ->orderBy('position', 'asc')
                ->get();/* ->get()->transform(function ($item) {
    ...
    {
        $view = 'admin.goods-page.edit-menu';
        $modules_name = $this->menu()['modules_name'];
        $url_for_active_elem = '/' . LANG . '/back/' . $modules_name->modulesId->alias;

        $menu_without_lang = GoodsPage::where('goods_page_id', $id)->first();

        $parent_menu_id = GoodsPageId::whereRaw('id IN(SELECT p_id FROM goods_page_id WHERE id = ' . $id . ')')
            ->first();
        ...
        $menu_elems = GoodsPage::where('lang_id', $lang_id)
            ->where('goods_page_id', $menu_without_lang->goods_page_id)
            ->first();

        if (!is_null($menu_without_lang)) {
            $menu_id = GoodsPageId::where('id', $menu_without_lang->goods_page_id)
                ->first();
        } elseif (!is_null($menu_elems)) {
            $menu_id = GoodsPageId::where('id', $menu_elems->goods_page_id)
                ->first();
        }
        ...
        $images = GoodsPageImages::where('goods_page_id', $id)
            ->orderBy('position', 'asc')
            ->get();
    ...
    public function saveMenu(Request $request, $id, $lang_id)
    {
        if (is_null($id)) {
            $item = Validator::make($request->all(), [
                'name' => 'required',
                'alias' => 'required|unique:goods_page_id',
            ...
            $item = Validator::make($request->all(), [
                'name' => 'required',
                'alias' => 'required|unique:goods_page_id,alias,' . $id,
                'controller' => 'not_in:controller|not_in:Controller|min:10',
                'uploaded_files' => 'nullable|max:10',
                'upload_files.*' => 'image|mimes:jpeg,png,jpg,gif,svg|max:2048',
            ], [
        ...
        $maxPosition = GetMaxPosition('goods_page_id');
        $level = GetLevel($request->input('p_id'), 'goods_page_id');

        if ($id) {
            $currentPosition = GetPosition('goods_page_id', $id);
...
//        Check if lang exist
        ...
        $menu_id = GoodsPageId::updateOrCreate(['id' => $id], [
        ...
        GoodsPage::updateOrCreate([
            'goods_page_id' => $menu_id->id,
        ...
        if ($request->file('upload_files') && $menu_id)
            uploadMultipleFiles($request->file('upload_files'), $request->input('uploaded_files'), 'goods-pages', $menu_id);
        ...
        if (is_null($id)) {
            if ($menu_id->level == 1) {
                ...
                return response()->json([
                    'status' => true,
                    'messages' => [controllerTrans('variables.save', LANG)],
                    'redirect' => urlForFunctionLanguage(LANG, GetParentAlias($menu_id->id, 'goods_page_id') . '/memberslist')
                ]);
            }
        }
        return response()->json([
    ...
    {
        $view = 'admin.goods-page.child-list';

        $modules_name = $this->menu()['modules_name'];
        $url_for_active_elem = '/' . LANG . '/back/' . $modules_name->modulesId->alias;

        $menu_list_id = GoodsPageId::where('alias', request()->segment(4))
            ->first();
        ...
        $child_menu_list_id = GoodsPageId::where('p_id', $menu_list_id->id)
            ->where('deleted', 0)
            ->orderBy('position', 'asc')
            ->paginate(config('custom.back.menu_items_per_page'));
        ...
        foreach ($child_menu_list_id as $key => $one_menu_elem) {
            $child_menu_list[$key] = GoodsPage::where('goods_page_id', $one_menu_elem->id)
                ->first();
        }
    ...
    {
        $view = 'admin.goods-page.menu-cart';

        $deleted_elems_by_alias = GoodsPageId::where('alias', request()->segment(4))
            ->first();

        if (is_null($deleted_elems_by_alias)) {
            $deleted_menu_id_elems = GoodsPageId::where('deleted', 1)
                ->where('active', 0)
                ->where('p_id', 0)
                ->get();
        } else {
        ...
        foreach ($deleted_menu_id_elems as $key => $one_deleted_menu_elem) {
            $deleted_menu_elems[$key] = GoodsPage::where('goods_page_id', $one_deleted_menu_elem->id)
                ->first();
        }
    ...
    public function destroyMenuToCart(Request $request)
    {
        ...
        if (!empty($deleted_elements_id)) {
            ...
            $menu_item_elems_id = GoodsPageId::whereIn('id', $deleted_elements_id_arr)->get();
            ...
            if (!$menu_item_elems_id->isEmpty()) {
                ...
                foreach ($menu_item_elems_id as $one_menu_item_elems_id) {
                    ...
                    $menu_item_elems = GoodsPage::where('goods_page_id', $one_menu_item_elems_id->id)
                    ...
                    if ($one_menu_item_elems_id->deleted == 0) {
                        ...
                        GoodsPageId::where('id', $one_menu_item_elems_id->id)
    ...
    public function restoreMenu(Request $request)
    {
        ...
        if (!empty($restored_elements_id)) {
            ...
            $promotion_item_elems_id = GoodsPageId::whereIn('id', $restored_elements_id_arr)->get();
            ...
            if (!$promotion_item_elems_id->isEmpty()) {
                ...
                foreach ($promotion_item_elems_id as $one_promotion_item_elems_id) {
                    ...
                    $promotion_name = GetNameByLang($one_promotion_item_elems_id->id, LANG_ID, 'GoodsPage', 'goods_page_id');

                    if ($one_promotion_item_elems_id->restored == 0) {

                        $cart_message .= $promotion_name . ', ';

                        GoodsPageId::where('id', $one_promotion_item_elems_id->id)
            ...
            if (!$menu_item_elems_id->isEmpty()) {
                ...
                foreach ($menu_item_elems_id as $one_menu_item_elems_id) {
                    ...
                    if ($one_menu_item_elems_id->deleted == 1 && $one_menu_item_elems_id->active == 0) {
                        ...
                        GoodsPageId::destroy($one_menu_item_elems_id->id);
                        GoodsPage::where('goods_page_id', $one_menu_item_elems_id->id)->delete();
