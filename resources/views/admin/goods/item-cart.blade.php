                ...
                <div class="col-xl-12 mx-auto">
                    <div class="card">
                        <div class="card-body">
                                ...
                                <div class="ms-auto">
                                    ...
                                    @if(!empty($deleted_item_elems))
                                        <button class="btn btn-success btn-sm mt-2 mt-lg-0 restore-all-elements"
                                                data-url="{{urlForFunctionLanguage($lang, 'restoreGoodsItem/restoreGoodsItem')}}"
                                        ...
                                        @if($groupSubRelations->del_to_rec == 1)
                                            <button class="btn btn-danger btn-sm mt-2 mt-lg-0 destroy-all-elements"
                                                    data-url="{{urlForFunctionLanguage($lang, 'destroyGoodsItemFromCart/destroyGoodsItemFromCart')}}"
                                                    data-current-url="{{ url()->current() }}" disabled>
                            ...
                            @if(!empty($deleted_item_elems) && count($deleted_item_elems))
                                <div class="table-responsive">
                                    <table class="table mb-0 table-hover">
                                        ...
                                        @foreach($deleted_item_elems as $deleted_one_item_elem)
                                            <tr id="{{$deleted_one_item_elem->goods_item_id}}">
                                                <th class="text-center" scope="row">{{ $loop->iteration }}</th>
                                                <td class="text-center">
                                                    <span>{{!empty(IfHasName($deleted_one_item_elem->goods_item_id, $lang_id, 'goods_item')) ? IfHasName($deleted_one_item_elem->goods_item_id, $lang_id, 'goods_item') : __('variables.another_name')}}</span>
                                                </td>
                                                <td class="text-center">
                                                    <input class="form-check-input restore-element" type="checkbox"
                                                           name="restore_element"
                                                           value="{{$deleted_one_item_elem->goods_item_id}}"
                                                           id="restore-element-{{$deleted_one_item_elem->goods_item_id}}">
                                                    <label class="form-check-label"
                                                           for="restore-element-{{$deleted_one_item_elem->goods_item_id}}"></label>
                                                </td>
                                                @if($groupSubRelations->del_to_rec == 1)
                                                    <td class="text-center">
                                                        <input class="form-check-input destroy-element" type="checkbox"
                                                               ...
                                                               value="{{$deleted_one_item_elem->goods_item_id}}"
                                                               id="destroy-element-{{$deleted_one_item_elem->goods_item_id}}">
                                                        <label class="form-check-label"
                                                               for="destroy-element-{{$deleted_one_item_elem->goods_item_id}}"></label>
                                                    </td>
                                                @endif
                                            </tr>
                                        @endforeach
