        ...
        <div class="page-content">
            ...
            <div class="card">
                <div class="card-body">
                    <div class="d-lg-flex align-items-center mb-4 gap-3">
                        <div class="ms-auto">
                            @if($groupSubRelations->new == 1)
                                ...
                                <a href="{{ urlForFunctionLanguage($lang, 'createGoodsTag/createGoodsTag') }}"
                                   class="btn btn-primary mt-2 mt-lg-0"><i
                            ...
                            @if($groupSubRelations->del_to_rec == 1)
                                <button class="btn btn-danger btn-sm mt-2 mt-lg-0 destroy-all-elements"
                                        data-url="{{urlForFunctionLanguage($lang, 'destroyGoodsTag/destroyGoodsTag')}}"
                    ...
                    <hr/>
                    @if(!empty($goods_tags_list) && count($goods_tags_list))
                        <div class="table-responsive tables-white-space-normal">
                            <table class="table mb-0 table-hover">
                                ...
                                @foreach($goods_tags_list as $one_goods_tag)
                                    <tr class="row-id" data-id="{{$one_goods_tag->id}}">
                                        <th class="text-center" scope="row">{{ $goods_tags_list->firstItem() + $loop->index }}</th>
                                        <td class="text-center">
                                            <span>{{ $one_goods_tag->itemByLang->name ?? '' }}</span>
                                        </td>
                                        <td class="text-center">
                                            <div class="d-flex ml-labels-buttons justify-content-center">
                                                @foreach($lang_list as $lang_key => $one_lang)
                                                    <a href="{{urlForFunctionLanguage($lang, Str::slug($one_goods_tag->itemByLang->name).'/editGoodsTag/'.$one_goods_tag->id.'/'.$lang_key)}}"
                                                       class="btn btn-sm btn-{{ !empty(IfHasName($one_goods_tag->id, $lang_key, 'goods_item_tags')) ? 'success' : 'danger' }}">{{Str::ucfirst($one_lang)}}</a>
                                                @endforeach
                                            </div>
                                        </td>
                                        <td class="text-center">
                                            <div class="form-switch">
                                                <input class="form-check-input change-active" type="checkbox"
                                                       data-active="{{$one_goods_tag->active}}"
                                                       data-element-id="{{$one_goods_tag->id}}"
                                                       data-action="main-active"
                                                       id="switch-active-{{$one_goods_tag->id}}"
                                                       data-url="{{$url_for_active_elem}}" {{$one_goods_tag->active == 1 ? 'checked' : ''}}>
                                                <label class="form-check-label"
                                                       for="switch-active-{{$one_goods_tag->id}}"></label>
                                            </div>
                                        </td>
                                        @if($groupSubRelations->del_to_rec == 1 || $groupSubRelations->del_from_rec == 1)
                                            <td class="text-center">
                                                <input class="form-check-input destroy-element" type="checkbox"
                                                       ...
                                                       value="{{$one_goods_tag->id}}"
                                                       id="destroy-element-{{$one_goods_tag->id}}">
                                                <label class="form-check-label"
                                                       for="destroy-element-{{$one_goods_tag->id}}"></label>
                                            </td>
                                        @endif
                                    </tr>
                                @endforeach
                        ...
                        @include('admin.templates.pagination', ['paginator' => $goods_tags_list])
                    @else
