                ...
                <div class="col-xl-12 mx-auto">
                    <div class="card">
                        <div class="card-body">
                                ...
                                <div class="ms-auto">
                                    ...
                                    @if(!empty($deleted_info_line))
                                        <button class="btn btn-success btn-sm mt-2 mt-lg-0 restore-all-elements"
                                                data-url="{{urlForFunctionLanguage($lang, 'restoreInfoLine/restoreInfoLine')}}"
                                        ...
                                        @if($groupSubRelations->del_to_rec == 1)
                                            <button class="btn btn-danger btn-sm mt-2 mt-lg-0 destroy-all-elements"
                                                    data-url="{{urlForFunctionLanguage($lang, 'destroyInfoLineFromCart/destroyInfoLineFromCart')}}"
                                                    data-current-url="{{ url()->current() }}" disabled>
                            ...
                            @if(!empty($deleted_info_line) && count($deleted_info_line))
                                        ...
                                        @foreach($deleted_info_line as $deleted_one_info_line)
                                            <tr id="{{$deleted_one_info_line->info_line_id}}">
                                                ...
                                                <td class="text-center">
                                                    <span>{{!empty(IfHasName($deleted_one_info_line->info_line_id, $lang_id, 'info_line')) ? IfHasName($deleted_one_info_line->info_line_id, $lang_id, 'info_line') : __('variables.another_name')}}</span>
                                                </td>
                                                <td class="text-center">
                                                    <input class="form-check-input restore-element" type="checkbox"
                                                           name="restore_element"
                                                           value="{{$deleted_one_info_line->info_line_id}}"
                                                           id="restore-element-{{$deleted_one_info_line->info_line_id}}">
                                                    <label class="form-check-label"
                                                           for="restore-element-{{$deleted_one_info_line->info_line_id}}"></label>
                                                </td>
                                                @if($groupSubRelations->del_to_rec == 1)
                                                    <td class="text-center">
                                                        <input class="form-check-input destroy-element" type="checkbox"
                                                               ...
                                                               value="{{$deleted_one_info_line->info_line_id}}"
                                                               id="destroy-element-{{$deleted_one_info_line->info_line_id}}">
                                                        <label class="form-check-label"
                                                               for="destroy-element-{{$deleted_one_info_line->info_line_id}}"></label>
                                                    </td>
                                                @endif
                                            </tr>
                                        @endforeach
