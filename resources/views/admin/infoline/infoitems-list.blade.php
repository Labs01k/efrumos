        ...
        <div class="page-content">
            ...
            <div class="card">
                <div class="card-body">
                        ...
                        <div class="ms-auto">
                            ...
                            @if($groupSubRelations->del_to_rec == 1)
                                <button class="btn btn-danger btn-sm mt-2 mt-lg-0 destroy-all-elements"
                                        data-url="{{urlForFunctionLanguage($lang, 'destroyInfoItemToCart/destroyInfoItemToCart')}}"
                    ...
                    <hr/>
                    @if(!empty($info_item) && count($info_item))
                        ...
                        <div class="table-responsive">
                            <table class="table mb-0 table-hover">
                                ...
                                <tbody class="sort-table" data-action="promo-list" data-url="{{ $url_for_active_elem }}">
                                @foreach($info_item as $key => $one_info_item)
                                    <tr class="row-id" data-id="{{$one_info_item->info_item_id}}">
                                        <th class="text-center" scope="row">{{ $loop->iteration }}</th>
                                        <td class="text-center">
                                            <span>{{ !empty(IfHasName($one_info_item->info_item_id, $lang_id, 'info_item')) ? IfHasName($one_info_item->info_item_id, $lang_id, 'info_item') : __('variables.another_name')}}</span>
                                        </td>
                                        <td class="text-center">
                                            @foreach($lang_list as $lang_key => $one_lang)
                                                <a href="{{urlForLanguage($lang, 'editinfoitem/'.$one_info_item->info_item_id.'/'.$lang_key)}}"
                                                   class="btn btn-sm btn-{{ !empty(IfHasName($one_info_item->info_item_id, $lang_key, 'info_item')) ? 'success' : 'danger' }}">{{Str::ucfirst($one_lang)}}</a>
                                            @endforeach
                                        </td>
                                        <td class="text-center">
                                            <div class="form-switch">
                                                <input class="form-check-input change-active" type="checkbox"
                                                       ...
                                                       data-element-id="{{$one_info_item->info_item_id}}"
                                                       data-action="info_item"
                                                       id="switch-active-{{$one_info_item->info_item_id}}"
                                                       data-url="{{$url_for_active_elem}}" {{$one_info_item->infoItemId->active == 1 ? 'checked' : ''}}>
                                                <label class="form-check-label"
                                                       for="switch-active-{{$one_info_item->info_item_id}}"></label>
                                            </div>
                                        </td>
                                        @if(request()->segment(4) == 'promo')
                                        ...
                                        <td class="text-center">
                                            <span
                                                class="badge bg-secondary">{{ getDefaultDateFormatAdmin($one_info_item->infoItemId->add_date) }}</span>
                                        </td>
                                        @if($groupSubRelations->del_to_rec == 1)
                                            <td class="text-center">
                                                <input class="form-check-input destroy-element" type="checkbox"
                                                       ...
                                                       id="destroy-element-{{$one_info_item->info_item_id}}">
                                                <label class="form-check-label"
                                                       for="destroy-element-{{$one_info_item->info_item_id}}"></label>
                                            </td>
                                        @endif
                                    </tr>
                                @endforeach
                        ...
                        @include('admin.templates.pagination', ['paginator' => $info_items_list])
                    @else
