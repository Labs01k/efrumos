                            ...
                            <div class="d-lg-flex align-items-center mb-4 gap-3">
                                <div class="ms-auto">
                                    ...
                                    @else
                                        ...
                                        @endif
                                    @endif
                                    @if(!empty($deleted_modules_elems) && count($deleted_modules_elems))
                                        <button class="btn btn-success btn-sm mt-2 mt-lg-0 restore-all-elements"
                                                data-url="{{urlForFunctionLanguage($lang, 'restoreModules/restoreModules')}}"
                                        ...
                                        @if($groupSubRelations->del_to_rec == 1)
                                            <button class="btn btn-danger btn-sm mt-2 mt-lg-0 destroy-all-elements"
                                                    data-url="{{urlForFunctionLanguage($lang, 'destroyModulesFromCart/destroyModulesFromCart')}}"
                                                    data-current-url="{{ url()->current() }}" disabled>
                                        ...
                                        @foreach($deleted_modules_elems as $deleted_one_modules_elem)
                                            <tr id="{{$deleted_one_modules_elem->modules_id}}">
                                                ...
                                                <td class="text-center">
                                                    <span>{{!empty(IfHasName($deleted_one_modules_elem->modules_id, $lang_id, 'modules')) ? IfHasName($deleted_one_modules_elem->modules_id, $lang_id, 'modules') : __('variables.another_name')}}</span>
                                                </td>
                                                <td class="text-center">
                                                    <input class="form-check-input restore-element" type="checkbox"
                                                           name="restore_element"
                                                           value="{{$deleted_one_modules_elem->modules_id}}"
                                                           id="restore-element-{{$deleted_one_modules_elem->modules_id}}">
                                                    <label class="form-check-label"
                                                           for="restore-element-{{$deleted_one_modules_elem->modules_id}}"></label>
                                                </td>
                                                @if($groupSubRelations->del_to_rec == 1)
                                                    <td class="text-center">
                                                        <input class="form-check-input destroy-element" type="checkbox"
                                                               ...
                                                               value="{{$deleted_one_modules_elem->modules_id}}"
                                                               id="destroy-element-{{$deleted_one_modules_elem->modules_id}}">
                                                        <label class="form-check-label"
                                                               for="destroy-element-{{$deleted_one_modules_elem->modules_id}}"></label>
                                                    </td>
                                                @endif
                                            </tr>
                                        @endforeach
