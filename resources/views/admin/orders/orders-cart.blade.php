                                ...
                                <div class="ms-auto">
                                    ...
                                    @if(!empty($orders) && count($orders))
                                        <button class="btn btn-success btn-sm mt-2 mt-lg-0 restore-all-elements"
                                                data-url="{{urlForFunctionLanguage($lang, 'restoreOrder/restoreOrder')}}"
                                        ...
                                        @if($groupSubRelations->del_to_rec == 1)
                                            <button class="btn btn-danger btn-sm mt-2 mt-lg-0 destroy-all-elements"
                                                    data-url="{{urlForFunctionLanguage($lang, 'destroyOrderFromCart/destroyOrderFromCart')}}"
                                                    data-current-url="{{ url()->current() }}" disabled>
                                        ...
                                        @foreach($orders as $one_order)
                                            <tr id="{{$one_order->id}}">
                                                ...
                                                <td class="text-center">
                                                    <span>{{$one_order->ordersUsers->name .' '. $one_order->ordersUsers->last_name ?? ''}}</span>
                                                </td>
                                                <td class="text-center">
                                                    <input class="form-check-input restore-element" type="checkbox"
                                                           name="restore_element"
                                                           value="{{$one_order->id}}"
                                                           id="restore-element-{{$one_order->id}}">
                                                    <label class="form-check-label"
                                                           for="restore-element-{{$one_order->id}}"></label>
                                                </td>
                                                @if($groupSubRelations->del_to_rec == 1)
                                                    <td class="text-center">
