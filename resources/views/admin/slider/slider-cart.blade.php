                                ...
                                <div class="table-responsive">
                                    <table class="table mb-0 table-hover">
                                        ...
                                        @foreach($deleted_banner_list as $deleted_banner)
                                            <tr id="{{$deleted_banner->banner_top_id}}">
                                                <th class="text-center" scope="row">{{ $loop->iteration }}</th>
                                                <td class="text-center">
                                                    <span>{{!empty(IfHasName($deleted_banner->banner_top_id, $lang_id, 'banner_top')) ? IfHasName($deleted_banner->banner_top_id, $lang_id, 'banner_top') : __('variables.another_name')}}</span>
                                                </td>
                                                <td class="text-center">
                                                    <input class="form-check-input restore-element" type="checkbox"
                                                           name="restore_element"
                                                           value="{{$deleted_banner->banner_top_id}}"
                                                           id="restore-element-{{$deleted_banner->banner_top_id}}">
                                                    <label class="form-check-label"
                                                           for="restore-element-{{$deleted_banner->banner_top_id}}"></label>
                                                </td>
                                                @if($groupSubRelations->del_to_rec == 1)
                                                    <td class="text-center">
                                                        <input class="form-check-input destroy-element" type="checkbox"
                                                               ...
                                                               value="{{$deleted_banner->banner_top_id}}"
                                                               id="destroy-element-{{$deleted_banner->banner_top_id}}">
                                                        <label class="form-check-label"
                                                               for="destroy-element-{{$deleted_banner->banner_top_id}}"></label>
                                                    </td>
                                                @endif
                                            </tr>
                                        @endforeach
