                ...
                <div class="col-xl-12 mx-auto">
                    <div class="card">
                        <div class="card-body">
                                ...
                                <div class="ms-auto">
                                    ...
                                    @if(!empty($deleted_subject_elems))
                                        <button class="btn btn-success btn-sm mt-2 mt-lg-0 restore-all-elements"
                                                data-url="{{urlForFunctionLanguage($lang, 'restoreGallerySubject/restoreGallerySubject')}}"
                                        ...
                                        @if($groupSubRelations->del_to_rec == 1)
                                            <button class="btn btn-danger btn-sm mt-2 mt-lg-0 destroy-all-elements"
                                                    data-url="{{urlForFunctionLanguage($lang, 'destroyGallerySubjectFromCart/destroyGallerySubjectFromCart')}}"
                                                    data-current-url="{{ url()->current() }}" disabled>
                            ...
                            @if(!empty($deleted_subject_elems) && count($deleted_subject_elems))
                                        ...
                                        @foreach($deleted_subject_elems as $deleted_one_subject_elem)
                                            <tr id="{{$deleted_one_subject_elem->gallery_subject_id}}">
                                                ...
                                                <td class="text-center">
                                                    <span>{{!empty(IfHasName($deleted_one_subject_elem->gallery_subject_id, $lang_id, 'gallery_subject')) ? IfHasName($deleted_one_subject_elem->gallery_subject_id, $lang_id, 'gallery_subject') : __('variables.another_name')}}</span>
                                                </td>
                                                <td class="text-center">
                                                    <input class="form-check-input restore-element" type="checkbox"
                                                           ...
                                                           id="restore-element-{{$deleted_one_subject_elem->gallery_subject_id}}">
                                                    <label class="form-check-label"
                                                           for="restore-element-{{$deleted_one_subject_elem->gallery_subject_id}}"></label>
                                                </td>
                                                @if($groupSubRelations->del_to_rec == 1)
                                                    <td class="text-center">
                                                        <input class="form-check-input destroy-element" type="checkbox"
                                                               ...
                                                               value="{{$deleted_one_subject_elem->gallery_subject_id}}"
                                                               id="destroy-element-{{$deleted_one_subject_elem->gallery_subject_id}}">
                                                        <label class="form-check-label"
                                                               for="destroy-element-{{$deleted_one_subject_elem->gallery_subject_id}}"></label>
                                                    </td>
                                                @endif
                                            </tr>
                                        @endforeach
