                                ...
                                <div class="ms-auto">
                                    ...
                                    @if(!empty($deleted_item_elems))
                                        <button class="btn btn-success btn-sm mt-2 mt-lg-0 restore-all-elements"
                                                data-url="{{urlForFunctionLanguage($lang, 'restoreGalleryItem/restoreGalleryItem')}}"
                                        ...
                                        @if($groupSubRelations->del_to_rec == 1)
                                            <button class="btn btn-danger btn-sm mt-2 mt-lg-0 destroy-all-elements"
                                                    data-url="{{urlForFunctionLanguage($lang, 'destroyGalleryItemFromCart/destroyGalleryItemFromCart')}}"
                                                    data-current-url="{{ url()->current() }}" disabled>
                                        ...
                                        @foreach($deleted_item_elems as $deleted_one_item_elem)
                                            <tr id="{{$deleted_one_item_elem->gallery_item_id}}">
                                                ...
                                                <td class="text-center">
                                                    <span>{{!empty(IfHasName($deleted_one_item_elem->gallery_item_id, $lang_id, 'gallery_item')) ? IfHasName($deleted_one_item_elem->gallery_item_id, $lang_id, 'gallery_item') : __('variables.another_name')}}</span>
                                                </td>
                                                <td class="text-center">
                                                    <input class="form-check-input restore-element" type="checkbox"
                                                           ...
                                                           id="restore-element-{{$deleted_one_item_elem->gallery_item_id}}">
                                                    <label class="form-check-label"
                                                           for="restore-element-{{$deleted_one_item_elem->gallery_item_id}}"></label>
                                                </td>
                                                @if($groupSubRelations->del_to_rec == 1)
                                                    <td class="text-center">
                                                        <input class="form-check-input destroy-element" type="checkbox"
                                                               ...
                                                               value="{{$deleted_one_item_elem->gallery_item_id}}"
                                                               id="destroy-element-{{$deleted_one_item_elem->gallery_item_id}}">
                                                        <label class="form-check-label"
                                                               for="destroy-element-{{$deleted_one_item_elem->gallery_item_id}}"></label>
                                                    </td>
                                                @endif
                                            </tr>
                                        @endforeach
