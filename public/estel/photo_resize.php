...
function CreateImageManipulator($relative_path_to_file, $relative_output_to_file, $file_name, $width, $height, $clip=false, $noenlarge=true){
	...
	$i = new ImageManipulator(DOC_ROOT.$relative_path_to_file.$file_name);
	$i->resize_to_fit($width, $height, $clip, $noenlarge);
	$punct_pos = mb_strrpos($file_name, ".");
  $extension = mb_substr($file_name, $punct_pos+1);
    switch ($extension){
    	case "jpg":
    		$i->save_jpeg(DOC_ROOT.$relative_output_to_file.$file_name);
    		break;

    	case "png":
    		$i->save_png(DOC_ROOT.$relative_output_to_file.$file_name);
    		break;
...
//$img=upload("/foto/", "foto");

//$i = new ImageManipulator(DOC_ROOT."/foto/".$img);
//$i->resize_to_fit(154,115,false,true);

//Эту часть можно опустить, она дорисовывает бордер
//$i->color = array(255,255,255);		//set foreground colour to white
//$i->draw_border(15);			//draw a 15px border
//$i->color = array(0,0,0);		//set foreground to black
//$i->draw_border(1);			//draw 1px border
//Эту часть можно опустить, она дорисовывает бордер

//$i->save_jpeg(DOC_ROOT."/foto/s/".$img);			//Есть также output_jpeg
